...
In our implementation we follow the ideas from Allan's article.
...
When we want to interact with REST SSE, we need to use the correct types, 
as atoms and tuples are not fully compatible with JSON. 
Sending a message via Elixir to be received by the above subscription looks as follows,
with a string topic name and a list instead of a tuple.

```elixir
iex>GenServer.call({:global, :ps1}, {:broadcast, "topic1", ["msg", "hi again!"]})
14:07:51.225 [debug] ["msg","hi again!"]
true
```

We can write the same as an Elixir test, though we need to use some Elixir magic
to overcome the blocking of the call-receive POST.
...
test "pub-sub genserver" do
  ...
  task = Task.async(fn ->
    Req.post!(
      ...
      into: fn {:data, data}, {req, resp} ->
        ...
        case data do
          <<"event: ready", _::binary>> ->
            send(test_pid, :ready)
            {:cont, {req, resp}}

          <<"data:", json::binary>> ->
            ["msg", 101] = JSON.decode!(json)
            send(test_pid, :msg_101)
            {:halt, {req, resp}}
  ...
  assert(
    Req.post!(
      "http://localhost:4000/process/ps1/call",
      json: ["broadcast", "topic1", ["msg", 101]]
    ).body
  )

  assert_receive :msg_101
