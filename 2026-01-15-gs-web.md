...
In our implementation we follow the ideas from the article mentioned earlier.
...
We remain in this loop forever, until the HTTP client close,
...
```console
iex(zed@pathfinder)> GenServer.call({:global, :ps1}, {:broadcast, :topic1, {:msg, :yo}})
true
```
