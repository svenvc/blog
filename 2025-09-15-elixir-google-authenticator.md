...
Once set up, the authenticator generates codes that are checked by the server. As long as the use the same secret they originally shared, their codes will match and authentication will be accepted.  

To exchange the key, it is encoded in Base32, resulting in a 32 character string. The QR code that you see includes this key.
